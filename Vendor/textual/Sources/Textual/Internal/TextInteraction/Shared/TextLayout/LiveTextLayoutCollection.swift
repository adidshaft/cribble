  ...
  final class SnapshotTextLayoutCollection: TextLayoutCollection {
    let layouts: [any TextLayout]

    private let baseLayouts: [Text.Layout]
    private let origins: [CGPoint]

    init(base: Text.LayoutKey.Value, geometry: GeometryProxy) {
      let anchoredLayouts = base.filter(\.layout.isTextFragment)
      baseLayouts = anchoredLayouts.map(\.layout)
      origins = anchoredLayouts.map { geometry[$0.origin] }
      layouts = zip(baseLayouts, origins).map { layout, origin in
        LiveTextLayout(base: layout, origin: origin)
      }
    }

    func isEqual(to other: any TextLayoutCollection) -> Bool {
      guard let other = other as? SnapshotTextLayoutCollection else { return false }
      return baseLayouts == other.baseLayouts && origins == other.origins
    }

    func needsPositionReconciliation(with other: any TextLayoutCollection) -> Bool {
      baseLayouts != (other as? SnapshotTextLayoutCollection)?.baseLayouts
    }

    func index(of layout: Text.Layout) -> Int? {
      baseLayouts.firstIndex(of: layout)
    }
  }
