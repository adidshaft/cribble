...
struct TextSelectionInteraction: ViewModifier {
    ...
    @State private var layoutUpdateGeneration = 0
  #endif
          ...
          .overlayTextLayoutCollection { layoutCollection in
            Color.clear
              .onChange(of: AnyTextLayoutCollection(layoutCollection), initial: true) {
                layoutUpdateGeneration += 1
                let generation = layoutUpdateGeneration
                DispatchQueue.main.async {
                  guard generation == layoutUpdateGeneration else { return }
