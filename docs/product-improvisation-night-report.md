...
- Quick-action extension IDs now use the same safe-token validation as other
  ...
  path-like values.
- Trust declaration source URLs now follow the same HTTPS-unless-localhost rule,
  so future executable provenance metadata does not point at insecure remote
  source or release pages.
...
- Bundled DemoNotes and the first-run starter checklist now also put Settings >
  Extensions > Copy Proposal before the Help proposal route, so in-app
  onboarding matches the Settings-first contributor flow.
...
- Importer manifests also reject duplicate file extensions case-insensitively,
  preventing noisy or ambiguous import-lane summaries such as `json` plus
  `JSON`.
...
- DemoNotes home and the first-run starter checklist now point extension PR
  authors to Settings > Extensions > Contribution Guide, matching the native
  authoring surface before they create manifests or copy proposals.
...
- CONTRIBUTING and the short product-improvisation summary now use the same
  Settings-first proposal, import-lane review, and Project Intelligence review
  language as the app handoff surfaces, with Help fallbacks still visible.
